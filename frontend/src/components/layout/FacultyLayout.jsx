...
import { 
  ...
  Star,
  FolderGit2
, User, Mail, Briefcase, Phone, MapPin, AlignLeft, Camera } from 'lucide-react';
...
const FacultyLayout = () => {
  ...
  const realUser = faculty.find(f => f.name === sessionUser.name) || sessionUser;
  ...
  const handleAvatarUpload = (e) => {
    ...
    if (file) {
      ...
      reader.onloadend = () => {
        ...
        localStorage.setItem('svcet_session_faculty', JSON.stringify(updatedUser));
        if (realUser.id) {
          updateFaculty(realUser.id, { avatar: base64String });
        }
  ...
  const navItems = [
    ...
    { name: 'My Students', path: '/faculty/students', icon: <Users className="w-5 h-5" /> },
    { name: 'Attendance', path: '/faculty/attendance', icon: <CalendarCheck className="w-5 h-5" /> },
    { name: 'Grades & Marks', path: '/faculty/grades', icon: <GraduationCap className="w-5 h-5" /> },
    { name: 'Assessments', path: '/faculty/assessments', icon: <FileCode className="w-5 h-5" /> },
    { name: 'Materials', path: '/faculty/materials', icon: <BookOpen className="w-5 h-5" /> },
    { name: 'Notices', path: '/faculty/notices', icon: <Bell className="w-5 h-5" /> },
    { name: 'Messages', path: '/faculty/messages', icon: <MessageSquare className="w-5 h-5" />, badge: unreadMessagesCount },
    { name: 'Requests', path: '/faculty/requests', icon: <ClipboardList className="w-5 h-5" /> },
    // Mentoring & Verification
    { name: 'Skill Verify', path: '/faculty/skill-verify', icon: <Star className="w-5 h-5" /> },
    { name: 'Student Projects', path: '/faculty/student-projects', icon: <FolderGit2 className="w-5 h-5" /> },
    { name: 'Mentorship Hub', path: '/faculty/mentorship', icon: <BookOpen className="w-5 h-5" /> },
    { name: 'Settings', path: '/faculty/settings', icon: <Settings2 className="w-5 h-5" /> },
  ];
  ...
  return (
    <>
          ...
          {!effectivelyCollapsed && (
            <div className="flex items-center gap-2 cursor-pointer transition-transform hover:scale-105" onClick={() => setIsCollapsed(!isCollapsed)} title={isCollapsed ? "Pin Sidebar" : "Collapse Sidebar"}>
              ...
              <div className="text-xl font-bold tracking-tight flex items-center">
                SVCET<span className="text-blue-500">faculty</span>
              </div>
            </div>
          )}
          {effectivelyCollapsed && (
      ...
      {/* Main Content Area */}
      <main className="flex-1 flex flex-col h-[100dvh] overflow-hidden relative bg-neutral-50 dark:bg-[#0a0a0a]">
        ...
        <header className="h-20 bg-white/40 dark:bg-neutral-950/40 backdrop-blur-2xl border-b border-white/20 dark:border-white/10 shadow-sm z-10 flex items-center justify-between px-6 z-10 sticky top-0">
          <div className="flex items-center gap-4">
            ...
            <h2 className="text-xl font-semibold hidden sm:block">Faculty Workspace</h2>
          </div>
          ...
          <div className="flex items-center gap-4">
            ...
            <button 
              ...
              className="h-9 w-9 rounded-full bg-gradient-to-tr from-indigo-500 to-purple-500 flex items-center justify-center text-white font-bold shadow-md hover:ring-2 hover:ring-indigo-500 hover:ring-offset-2 dark:hover:ring-offset-neutral-900 transition-all cursor-pointer overflow-hidden"
            >
              {sessionUser?.avatar ? (
        ...
        <Dialog open={isProfileOpen} onOpenChange={setIsProfileOpen}>
          <DialogContent className="sm:max-w-md backdrop-blur-xl bg-white/70 dark:bg-neutral-900/70 border-white/20 dark:border-neutral-800/50 shadow-2xl">
            <DialogHeader>
              <DialogTitle className="text-center text-xl font-bold">Faculty Profile</DialogTitle>
            </DialogHeader>
            <div className="mt-4 flex flex-col items-center">
              <div className="relative group">
                <div className="h-24 w-24 rounded-full bg-gradient-to-tr from-indigo-500 to-purple-500 flex items-center justify-center text-white font-bold text-3xl shadow-lg ring-4 ring-indigo-50 dark:ring-indigo-900/30 mb-4 overflow-hidden">
                  {realUser?.avatar ? (
                ...
                <label className="absolute bottom-4 right-0 bg-purple-600 text-white p-1.5 rounded-full shadow-lg cursor-pointer hover:bg-purple-700 transition-colors opacity-0 group-hover:opacity-100">
                  <Camera className="w-4 h-4" />
                  <input type="file" accept="image/*" className="hidden" onChange={handleAvatarUpload} />
                </label>
              </div>
              <h2 className="text-2xl font-bold text-neutral-900 dark:text-white">{realUser?.name || 'Loading...'}</h2>
              <p className="text-indigo-600 dark:text-indigo-400 font-medium flex items-center gap-2 mt-1">
                <Briefcase className="w-4 h-4" /> {realUser?.designation || 'Faculty Member'}
              </p>
            </div>
            <div className="space-y-4 mt-6">
              ...
              {realUser?.office && (
                <div className="flex items-center gap-3 p-3 rounded-lg bg-white/40 dark:bg-neutral-800/40 backdrop-blur-sm border border-white/30 dark:border-neutral-700/30">
                  ...
                  <div>
                    <p className="text-xs text-neutral-500">Office Location & Hours</p>
                    <p className="text-sm font-medium">{realUser.office}</p>
                  </div>
                </div>
              )}
              {realUser?.bio && (
                <div className="flex items-start gap-3 p-3 rounded-lg bg-white/40 dark:bg-neutral-800/40 backdrop-blur-sm border border-white/30 dark:border-neutral-700/30">
                  <AlignLeft className="w-5 h-5 text-neutral-500 shrink-0 mt-0.5" />
                  <div>
                    <p className="text-xs text-neutral-500">Bio</p>
                    <p className="text-sm font-medium">{realUser.bio}</p>
                  </div>
                </div>
              )}
            </div>
